...
function showPage(whichPage, movieData, limitPerPage) {
    var totalPages = Math.ceil(movieData.length / limitPerPage);
...
function displayMovieData(movies, page = 1) {
    ...
    $(".movie-wrapper").innerHTML(displaydata);
    showPage(page, movies, limitPerPage);
}
...
$(function () {
    ...
    const categories = movieData.reduce(function (values, item) {
    ...
    btnSearch.addEventListener("click", (e) => {
        ...
        if (searchValue !== "") {
            let searchCategory = movieData.filter(function (data) {
                if (data.title.toLowerCase().includes(searchValue.toLowerCase()) || data.genre.toLowerCase().includes(searchValue.toLowerCase())) {
                    return data;
                }
            });
    ...
    $(".movie-wrapper").show();
    showPage(1, movieData, limitPerPage);

    $(document).on("click", ".pagination li.current-page:not(.active)", function () {
        return showPage(+$(this).text(), movieData, limitPerPage);
    });

    $(".next-page").on("click", function () {
        return showPage(currentPage + 1, movieData, limitPerPage);
    });
...
//             if (genre === "All") {
//                 displayMovieData(movieData, 1);
//             } else {
//                 displayMovieData(movieGenre, 1);
//             }
//         });
//     });
...
//             });
//             if (searchCategory) {
//                 displayMovieData(searchCategory, 1);
//             }
